...
### The persistence rule — the most consequential piece here

At the per-state $\alpha = 0.05$ calibration `surprise.flag` provides, each tick is an independent
test. Over a ~150-tick healthy trial, the probability of **at least one** exceedance is

$$
1 - (1-\alpha)^{T} \;=\; 1 - 0.95^{150} \;\approx\; 0.9994 .
$$

So "flag the trial if any tick fires" gives a ~100% trial-level false-positive rate **purely from
arithmetic** — measured directly at ~100% for *both* the cascade and joint models, confirming it is
a property of scoring every tick independently with no persistence requirement, not a per-channel
or per-model calibration bug.

`_persistent_mask` fixes this by collapsing the union flag mask to ticks belonging to a run of
$\ge$ `min_run` **consecutive** flagged ticks. Sweeping the longest-flagged-run distribution of
healthy trials down to a ~10% trial-level rate gave `min_run = 10` (cascade 10.0%, joint 6.2% at
$n = 80$) — the smallest persistence requirement that gets both models into a reasonable band.

**The rule is applied asymmetrically, and that asymmetry is the point:**

| Where | Rule | Why |
|---|---|---|
| **inside** the window | single-tick sensitive, `min_run` ignored | there *is* an injected anomaly; delaying detection by 10 ticks would destroy the measured 0-tick substitution latency |
| **outside** the window, and on healthy trials | requires a run of $\ge$ `min_run` | there is nothing to detect, so persistence is the right bar |

### The reported numbers

For each error type, with $\text{TP}$ = degraded trials detected in-window,
$\text{FP}_{\text{out}}$ = degraded trials with a persistent run outside their window,
$\text{FP}_{\text{healthy}}$ = healthy trials with a persistent run anywhere:
